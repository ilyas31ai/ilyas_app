...
class LyceeDashboardPage extends StatelessWidget {
  const LyceeDashboardPage({super.key});
...
class _Body extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: Text(
          _classeNom.isNotEmpty ? 'Lycée · $_classeNom' : 'Espace Lycée',
                ...
                if (_classeNom.isNotEmpty) ...[
                  ...
                  Row(
                    children: [
                      ...
                      TextButton(
                        onPressed: () => Navigator.pushNamed(context, AppRoutes.lyceeDevoirs),
                        child: const Text('Voir tout', style: TextStyle(color: Color(0xFF6C47FF), fontSize: 12)),
                      ),
                    ],
                  ),
                ...
                ...[
                  ...
                  Row(
                    children: [
                      ...
                      TextButton(
                        onPressed: () => Navigator.pushNamed(context, AppRoutes.lyceeNotes),
                        child: const Text('Voir tout', style: TextStyle(color: Color(0xFF6C47FF), fontSize: 12)),
                      ),
                    ],
                  ),
  ...
  Widget _buildQuickGrid(BuildContext context) {
    const items = [
      _NavItem(Icons.book_outlined, 'Matières', AppRoutes.lyceeMatieres, _kColors),
      _NavItem(Icons.assignment_outlined, 'Devoirs', AppRoutes.lyceeDevoirs, [Color(0xFF7C3AED), Color(0xFF6C47FF)]),
      _NavItem(Icons.grade_outlined, 'Notes', AppRoutes.lyceeNotes, [Color(0xFFD97706), Color(0xFFDC2626)]),
      _NavItem(Icons.description_outlined, 'Bulletin', AppRoutes.lyceeBulletin, [Color(0xFF15803D), Color(0xFF16A34A)]),
      _NavItem(Icons.emoji_events_outlined, 'BAC', AppRoutes.lyceeBacBlanc, [Color(0xFFBE185D), Color(0xFF9D174D)]),
      _NavItem(Icons.star_outlined, 'Spécialités', AppRoutes.lyceeSpecialites, [Color(0xFFD97706), Color(0xFFBE185D)]),
      _NavItem(Icons.how_to_reg_outlined, 'Présences', AppRoutes.lyceePresences, [Color(0xFF0F766E), Color(0xFF15803D)]),
      _NavItem(Icons.track_changes_outlined, 'Contrôle C.', AppRoutes.lyceeControleConu, [Color(0xFF0891B2), Color(0xFF2563EB)]),
      _NavItem(Icons.folder_outlined, 'Documents', AppRoutes.lyceeDocuments, [Color(0xFF374151), Color(0xFF1E3A5F)]),
    ];
...
class _Banner extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final first = user.displayName.isNotEmpty ? user.displayName.split(' ').first : 'Lycéen';
    return Container(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  classeNom.isNotEmpty ? 'Classe : $classeNom · Cycle Secondaire' : 'Cycle Secondaire',
                ...
                Container(
                  ...
                  child: const Text('Préparation BAC · Spécialités · Orientation', style: TextStyle(color: Colors.white, fontSize: 10)),
                ),
              ],
            ),
          ),
...
class _KpisRow extends StatelessWidget {
  final String classeNom;
  const _KpisRow({required this.classeNom});
      ...
      children: [
        Expanded(
          child: StreamBuilder<List<EmploiSlot>>(
            stream: classeNom.isNotEmpty ? EtudiantService.emploiStream(classeNom) : Stream.value([]),
            builder: (_, s) {
        ...
        Expanded(
          child: StreamBuilder<int>(
            stream: classeNom.isNotEmpty ? EtudiantService.devoirsEnAttenteStream(classeNom) : Stream.value(0),
            builder: (_, s) => _KpiBox(label: 'Devoirs\nen attente', value: s.data?.toString() ?? '–', color: const Color(0xFF7C3AED)),
          ),
        ),
        const SizedBox(width: 8),
        Expanded(
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Text('${slot.heureDebut}–${slot.heureFin}',
...
class _DevoirRow extends StatelessWidget {
  ...
  Color get _color => isLate ? Colors.red : isSoon ? Colors.orange : _kColors.first;

  @override
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () => Navigator.pushNamed(context, AppRoutes.lyceeDevoirs),
          ...
          children: [
            ...
            if (devoir.dateLimite != null)
              Column(
                ...
                children: [
                  Text('${devoir.dateLimite!.day}/${devoir.dateLimite!.month}',
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(note.intitule.isNotEmpty ? note.intitule : 'Note',
...
class _Loading extends StatelessWidget {
  ...
  Widget build(BuildContext context) => const Padding(
    ...
    child: Center(child: CircularProgressIndicator(color: Color(0xFF6C47FF), strokeWidth: 2)),
  );
}
