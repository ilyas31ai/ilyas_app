...
class CollegeMatieresPage extends StatelessWidget {
  const CollegeMatieresPage({super.key});
...
class _MatieresList extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Charge emploi du temps + notes en parallèle
      ...
      builder: (_, emploiSnap) => StreamBuilder<List<NoteModel>>(
        ...
        builder: (_, notesSnap) {
          ...
          for (final n in allNotes) {
            (notesByMatiere[n.matiere] ??= []).add(n);
          }

          // Matières connues depuis l'emploi du temps
          final matieresSlotsMap = <String, List<EmploiSlot>>{};
          for (final s in slots) {
            if (s.matiere.isNotEmpty) (matieresSlotsMap[s.matiere] ??= []).add(s);
          }

          // Ajoute aussi les matières avec des notes mais pas dans l'emploi du temps
          for (final matiere in notesByMatiere.keys) {
            matieresSlotsMap.putIfAbsent(matiere, () => []);
          }
          ...
          // Calcul de la moyenne générale (sur les matières avec notes)
            ...
            children: [
              ...
              ...matieres.asMap().entries.map((e) => Padding(
                ...
                child: _MatiereCard(
                  ...
                  color: _kMatiereColors[e.key % _kMatiereColors.length],
                ),
              )),
            ],
          );
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: _kColors.map((c) => c.withValues(alpha: 0.8)).toList(),
...
class _MatiereCard extends StatelessWidget {
  ...
  int get _nbHeures => slots.length;
              ...
              children: [
                if (moyenne != null)
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: _moyColor.withValues(alpha: 0.15),
                ...
                if (_nbHeures > 0)
                  Padding(
                    ...
                    child: Text('${_nbHeures}h/sem', style: const TextStyle(color: Colors.white24, fontSize: 10)),
                  ),
              ],
            ),
...
// ── Feuille détail notes par matière ─────────────────────────────────────────
                          ...
                          children: [
                            ...
                            Expanded(
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text(n.intitule.isNotEmpty ? n.intitule : 'Note', style: const TextStyle(color: Colors.white, fontSize: 13), maxLines: 1, overflow: TextOverflow.ellipsis),
                                  Text('${n.date.day}/${n.date.month}/${n.date.year}', style: const TextStyle(color: Colors.white38, fontSize: 11)),
                                ],
                              ),
                            ),
