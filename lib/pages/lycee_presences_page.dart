...
const _kColors = [Color(0xFF1E3A5F), Color(0xFFDC2626)];

enum _Statut { present, absent, retard }

class LyceePresencesPage extends StatelessWidget {
  const LyceePresencesPage({super.key});
...
class _Body extends StatelessWidget {
              ...
              builder: (_, snap) {
                ...
                final records = docs.map((d) {
                  ...
                  return (
                    ...
                    statut: _parseStatut(data['statut'] as String? ?? 'present'),
                    matiere: data['matiere'] as String? ?? '',
                  );
                }).toList();

                final nbPresent = records.where((r) => r.statut == _Statut.present).length;
                final nbAbsent = records.where((r) => r.statut == _Statut.absent).length;
                final nbRetard = records.where((r) => r.statut == _Statut.retard).length;
                final tauxPct = records.isEmpty ? 0 : (nbPresent * 100 ~/ records.length);

                return ListView(
                  padding: const EdgeInsets.fromLTRB(16, 12, 16, 32),
                  children: [
                    _StatsHeader(nbPresent: nbPresent, nbAbsent: nbAbsent, nbRetard: nbRetard, tauxPct: tauxPct),
                    const SizedBox(height: 16),
                    ...records.map((r) => _PresenceRow(date: r.date, statut: r.statut, matiere: r.matiere)),
                  ],
                );
              },
            ),
  ...
  static _Statut _parseStatut(String s) {
...
class _StatsHeader extends StatelessWidget {
  final int nbPresent, nbAbsent, nbRetard, tauxPct;
  const _StatsHeader({required this.nbPresent, required this.nbAbsent, required this.nbRetard, required this.tauxPct});
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Text('$tauxPct %', style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold)),
            ],
          ),
          const SizedBox(height: 12),
          Row(
            children: [
              _Chip('Présent', nbPresent.toString(), const Color(0xFF16A34A)),
              const SizedBox(width: 8),
              _Chip('Absent', nbAbsent.toString(), const Color(0xFFDC2626)),
              const SizedBox(width: 8),
              _Chip('Retard', nbRetard.toString(), const Color(0xFFD97706)),
            ],
          ),
        ],
      ),
...
class _PresenceRow extends StatelessWidget {
  ...
  const _PresenceRow({required this.date, required this.statut, required this.matiere});
            ...
            child: Column(
              ...
              children: [
                ...
                if (matiere.isNotEmpty)
                  Text(matiere, style: const TextStyle(color: Colors.white38, fontSize: 12)),
              ],
            ),
          ),
          Container(
