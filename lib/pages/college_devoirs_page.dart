...
class CollegeDevoirs extends StatelessWidget {
  const CollegeDevoirs({super.key});
...
class _SummaryBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: [
          _SummaryChip(label: 'À faire', count: aFaire, color: const Color(0xFF6C47FF)),
...
class _DevoirTile extends StatelessWidget {
  ...
  Color get _iconColor => switch (statut) {
    ...
    _Statut.aFaire   => _isUrgent ? Colors.orange : const Color(0xFF6C47FF),
  };
  ...
  Widget build(BuildContext context) {
    return InkWell(
      onTap: statut != _Statut.rendu ? () => _naviguerVersDevoir(context) : null,
          ...
          children: [
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(devoir.titre, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 14)),
                    ...
                    Padding(
                      ...
                      child: Row(
                        children: [
                          ...
                          Text(
                            'Limite : ${_fmt(devoir.dateLimite!)}',
                            style: TextStyle(color: _iconColor, fontSize: 11),
                          ),
                        ],
                      ),
                    ),
                  // Info soumission si rendu
                  if (submission != null) ...[
                    const SizedBox(height: 4),
  ...
  void _naviguerVersDevoir(BuildContext context) {
    Navigator.pushNamed(context, '/etudiant_faire_devoir', arguments: {
      'devoirId': devoir.id,
